...
public:
    AltitudeAttitudeControl()
        : Node("altitude_attitude_control"), counter_(0),
          ...
          pid_throttle_(0.1f, 0.5f, 0.6f),  // Direct altitude-to-throttle control
          pid_altitude_pitch_(0.02f, 0.005f, 0.1f),  // New altitude-to-pitch controller
...
private:
    ...
    int counter_;
    PID pid_yaw_;
    PID pid_throttle_;  // Direct altitude-to-throttle control (renamed for clarity)
    PID pid_altitude_pitch_;  // New PID for altitude-to-pitch mapping
    ...
    // Max pitch angle for altitude control (radians) - limiting to about 15 degrees
    const float max_pitch_for_altitude_ = 0.26f;
    ...
    // Control computation for roll and pitch using ASMC, with cascaded PID for altitude
    void publish_actuator_values(float dt) {
        // ------------ Altitude Control (Outer Loop) ------------
        float altitude_error = desired_altitude_ - current_altitude_;
        
        // Calculate desired pitch from altitude error (pitch up if too low)
        float pitch_from_altitude = pid_altitude_pitch_.compute(altitude_error, dt);
        
        // Limit pitch command to safe values
        pitch_from_altitude = std::clamp(pitch_from_altitude, -max_pitch_for_altitude_, max_pitch_for_altitude_);
        
        // Set desired pitch for inner loop controller
        desired_pitch_ = pitch_from_altitude;
        
        // Also compute throttle directly from altitude error
        float throttle = pid_throttle_.compute(altitude_error, dt);
        throttle = std::clamp(throttle, 0.3f, 1.0f);  // Limit throttle
        
        // -------- Roll channel (inner loop) -----------
        ...
        // Get current adapted gains based on error
        float K_roll = asmc_roll_.get_K(error_roll);

        // Compute control with fixed A and adaptive K (corrected formula)
        float roll_cmd = (asmc_roll_.A * s_roll + K_roll * sat_sign(s_roll, asmc_roll_.epsilon));
        prev_roll_ = current_roll_;

        // -------- Pitch channel (inner loop) -----------
        ...
        // Compute control with fixed A and adaptive K (corrected formula)
        float pitch_cmd = -(asmc_pitch_.A * s_pitch + K_pitch * sat_sign(s_pitch, asmc_pitch_.epsilon));
        ...
        // Enhanced logging to show altitude control
        RCLCPP_INFO(this->get_logger(),
                   "Alt tgt: %.2f | Cur: %.2f | Thr: %.2f | Cmd: %.2f | Pitch: %.2f",
                   desired_altitude_, current_altitude_, throttle,
                   pitch_cmd, desired_pitch_);
    }
    ...
    // Modified arm function to reset PIDs
    void arm() {
        // Reset PID controllers when arming to prevent integrator windup
        pid_altitude_pitch_.reset();
        pid_throttle_.reset();
